<template>
  <div class="home_view">
    <div class="title" style="margin-bottom: 14px">
      <el-text class="mx-1" size="large">商品标签</el-text>
    </div>
    <div class="cont">
          ...
          <el-form-item label="商品规格id : ">
            <div class="Landscape">
              <el-input
                style="width: 175px"
                clearable
                v-model.trim.number="form.product_spec_id"
                type="text"
              />
            </div>
          </el-form-item>
          <el-form-item label="标签id : ">
            <div class="Landscape">
              <el-input
                style="width: 175px"
                clearable
                v-model.trim.number="form.tag_id"
              />
            </div>
          </el-form-item>
    ...
    <el-dialog v-model="dialogTableVisible" title="Shipping address">
      <el-form
        ...
        ref="AddFormRef"
      >
        <el-form-item label="商品规格id" prop="product_spec_id">
          <el-input clearable v-model.trim.number="AddForm.product_spec_id" />
        </el-form-item>
        <el-form-item label="标签id" prop="tag_id">
          <el-input clearable v-model.trim.number="AddForm.tag_id" />
        </el-form-item>
...
import { addProductTag } from "@/api/add";
import { deleteProductTag } from "@/api/delete";
import { selectProductTag } from "@/api/home";
...
const getInfo = async () => {
  try {
    loading.value = true;
    const res = await selectProductTag(form.value);
    if (res.data.code === 0) {
...
const rules = ref({
  product_spec_id: [
    { required: true, message: "商品规格id不能为空", trigger: "change" },
    { pattern: /^\d+$/, message: "商品规格id只能输入数字", trigger: "change" },
  ],
  tag_id: [
    { required: true, message: "标签id不能为空", trigger: "change" },
    { pattern: /^\d+$/, message: "标签id只能输入数字", trigger: "change" },
  ],
});
...
const deleteInfo = async (id: number) => {
  ...
  try {
    loading.value = true;
    const res = await deleteProductTag({ product_tag_id: id });
    if (res.data.code === 0) {
